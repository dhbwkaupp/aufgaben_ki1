...
$\mathbf{x} = (x_0, x_1, x_2)^{\mathsf{T}}$ mit $x_0 = 1$ und das
Startgewicht
\[
  \mathbf{w}^{(0)} = (-0{,}2;\; 0{,}5;\; -0{,}3)^{\mathsf{T}},
  \qquad \eta = 0{,}2 .
\]
Die Ausgabe ist $\hat{y} = \text{heaviside}(\mathbf{w}\cdot\mathbf{x})$, d.\,h.
$\hat{y} = 1$ für $\mathbf{w}\cdot\mathbf{x} > 0$, sonst $\hat{y} = 0$.

Führen Sie \textbf{zwei aufeinanderfolgende} Schritte der Hebbschen Lernregel
$\Delta w_j = \eta\cdot\text{error}\cdot x_j$ mit $\text{error} = y - \hat{y}$ durch.
Verwenden Sie dabei jeweils das aus dem vorherigen Schritt aktualisierte
Gewicht als Startpunkt:

\begin{enumerate}[label=\alph*)]
  \item \textbf{Schritt 1:} Trainingsbeispiel $\mathbf{x} = (1, 1, 1)^{\mathsf{T}}$
        mit Zielwert $y = 1$. Bestimmen Sie $\hat{y}$, das error sowie das
        aktualisierte Gewicht $\mathbf{w}^{(1)}$.
  \item \textbf{Schritt 2:} Trainingsbeispiel $\mathbf{x} = (1, 1, 0)^{\mathsf{T}}$
        mit Zielwert $y = 0$, ausgehend von $\mathbf{w}^{(1)}$. Bestimmen Sie
        $\hat{y}$, das error sowie das aktualisierte Gewicht $\mathbf{w}^{(2)}$.
  \item Geben Sie die resultierende Trenngerade auf Basis von $\mathbf{w}^{(2)}$
        in der Form $x_2 = a\,x_1 + b$ an.
\end{enumerate}
}

\loesung{
\begin{enumerate}[label=\alph*)]
  \item \textbf{Schritt 1} mit $\mathbf{x} = (1,1,1)^{\mathsf{T}}$, $y = 1$,
  Startgewicht $\mathbf{w}^{(0)} = (-0{,}2;\,0{,}5;\,-0{,}3)^{\mathsf{T}}$:
  \[
    \mathbf{w}^{(0)}\cdot\mathbf{x} = -0{,}2 + 0{,}5 - 0{,}3 = 0
    \;\Rightarrow\; \hat{y} = 0, \qquad
    \text{error} = y - \hat{y} = 1 - 0 = 1 .
  \]
  Gewichtsänderung je Komponente:
  \[
    \Delta w_j = \eta\cdot\text{error}\cdot x_j = 0{,}2\cdot 1\cdot x_j
    \;\Rightarrow\;
    \Delta w_0 = 0{,}2,\; \Delta w_1 = 0{,}2,\; \Delta w_2 = 0{,}2 .
  \]
  Neues Gewicht:
  \[
  ...
  \item \textbf{Schritt 2} mit $\mathbf{x} = (1,1,0)^{\mathsf{T}}$, $y = 0$,
  Startgewicht $\mathbf{w}^{(1)} = (0;\,0{,}7;\,-0{,}1)^{\mathsf{T}}$
  (Ergebnis aus Schritt 1):
  \[
    \mathbf{w}^{(1)}\cdot\mathbf{x} = 0 + 0{,}7 + 0 = 0{,}7 > 0
    \;\Rightarrow\; \hat{y} = 1, \qquad
    \text{error} = y - \hat{y} = 0 - 1 = -1 .
  \]
  Gewichtsänderung je Komponente:
  \[
    \Delta w_j = 0{,}2\cdot(-1)\cdot x_j
    \;\Rightarrow\;
    \Delta w_0 = -0{,}2,\; \Delta w_1 = -0{,}2,\; \Delta w_2 = 0 .
  \]
  Neues Gewicht:
  \[
    \mathbf{w}^{(2)} = \mathbf{w}^{(1)} + \Delta\mathbf{w}
    = (-0{,}2;\; 0{,}5;\; -0{,}1)^{\mathsf{T}} .
  \]

  \item Trenngerade aus $\mathbf{w}^{(2)}$:
  \[
    -0{,}2 + 0{,}5x_1 - 0{,}1x_2 = 0
    \quad\xrightarrow{\cdot\,10}\quad
    5x_1 - x_2 - 2 = 0
